...
public:
    ...
    _Path_iterator& operator++() {
        ...
        _STL_VERIFY(_Myptr && _Myoff < _Myptr->m_str.size(), "path::iterator is not incrementable");
#endif // _ITERATOR_DEBUG_LEVEL != 0

        const size_t _Pend = _Myptr->_Prefix_end();
        const size_t _Size = _Myptr->m_str.size();

        if (_Myoff < _Pend) {
            _Myoff = _Pend; // point past "x:"
        }
        else if (_Myoff == _Pend && _Pend < _Size
            && _FS_ISSEP(_Myptr->m_str[_Pend])) { // point past root "/" and following slashes
            for (++_Myoff; _Myoff < _Size; ++_Myoff) {
                if (!_FS_ISSEP(_Myptr->m_str[_Myoff])) {
                    break;
                }
            }
        }
...
private:
    void _Getval() { // determine _Myelem
        ...
        if (_Size > _Myoff) {
            if (_Myoff < _Pend) {
                _Str = _Myptr->m_str.substr(0, _Pend); // get "x:"
            }
            else if (_Myoff == _Pend && _Pend < _Size && _FS_ISSEP(_Myptr->m_str[_Pend])) {
                _Str = _FS_PREF; // get "/"
            }
                ...
                for (; _Myoff + _Nslash < _Size; ++_Nslash) {
                    if (!_FS_ISSEP(_Myptr->m_str[_Myoff + _Nslash])) {
                        break;
                    }
                }

                for (; _Myoff + _Nslash + _Nstuff < _Size; ++_Nstuff) {
                    if (_FS_ISSEP(_Myptr->m_str[_Myoff + _Nslash + _Nstuff])) {
                        break;
                    }
                }

                if (0 < _Nstuff) {
                    _Str = _Myptr->m_str.substr(_Myoff + _Nslash, _Nstuff); // get "stuff"
                }
                else if (0 < _Nslash) {
                    _Str = _FS_PERIOD; // get "."
                }
...
public:
    ...
    path(const path& _Right) : m_str(_Right.m_str) {}
    ...
    path(path&& _Right) noexcept : m_str(_STD move(_Right.m_str)) {}

    path& operator=(path&& _Right) noexcept {
        m_str = _STD move(_Right.m_str);
        return *this;
    }
    ...
    path& operator/=(const path& _Path) { // append copy
        return append(_Path.m_str);
    }

    template <class _InIt>
    path& operator/=(_InIt _First) { // append NTCTS, given iterator
        return append(path(_First).m_str);
    }
    ...
    path& append(_InIt _First, _InIt _Last) { // append NTCTS
        return append(path(_First, _Last).m_str);
    }
    ...
    path& append(const basic_string<_Elem, _Traits, _Alloc>& _Str0) { // append arbitrary source string
        ...
        if (0 < m_str.size() && 0 < _Str.size() && m_str.back() != _FS_COLON && !_FS_ISSEP(m_str.back())
            && !_FS_ISSEP(_Str[0])) {
            m_str.push_back(_FS_PREF); // add needed separator
        }
    ...
    path& concat(_InIt _First, _InIt _Last) { // concatenate NTCTS
        return concat(path(_First, _Last).m_str);
    }
    ...
    path& make_preferred() { // convert any '/' to '\'
        _STD replace(m_str.begin(), m_str.end(), _FS_SLASH, _FS_BSLASH);
        return *this;
    }
    ...
    path& remove_filename() { // remove filename (leaf)
        if (!empty() && begin() != --end()) { // leaf to remove, back up over it
            ...
            size_t _Idx = m_str.size();

            for (; _Rend < _Idx; --_Idx) {
                if (_FS_ISSEP(m_str[_Idx - 1])) {
    ...
    _NODISCARD basic_string<_Elem, _Traits, _Alloc> string(
        ...
        return _Path_cvt<value_type, _Elem>::_Cvt(_Str, m_str.c_str(), m_str.size());
    }

    _NODISCARD _STD string string() const { // return path as basic_string<char> native
        _STD string _Str;
        return _Path_cvt<value_type, char>::_Cvt(_Str, m_str.c_str(), m_str.size());
    }

    _NODISCARD _STD wstring wstring() const { // return path as basic_string<wchar_t> native
        _STD wstring _Str;
        return _Path_cvt<value_type, wchar_t>::_Cvt(_Str, m_str.c_str(), m_str.size());
    }
    ...
    _NODISCARD _STD string u8string() const { // return path as basic_string<char> UTF8
        ...
        return _Path_cvt<value_type, _Char8_t, char_traits<char>, allocator<char>>::_Cvt(
            _Str, m_str.c_str(), m_str.size());
    }

    _NODISCARD _STD u16string u16string() const { // return path as basic_string<char16_t> UTF16
        _STD u16string _Str;
        return _Path_cvt<value_type, char16_t>::_Cvt(_Str, m_str.c_str(), m_str.size());
    }
    ...
    { // return path as basic_string<char32_t> UTF32
        _STD u32string _Str;
        return _Path_cvt<value_type, char32_t>::_Cvt(_Str, m_str.c_str(), m_str.size());
    }
#endif // 0
    ...
    _NODISCARD path root_name() const {
        return path(m_str.substr(0, _Prefix_end()));
    }

    _NODISCARD path root_directory() const {
        const size_t _Idx = _Prefix_end();
        if (_Idx < m_str.size() && _FS_ISSEP(m_str[_Idx])) {
    ...
    _NODISCARD path root_path() const {
        return path(m_str.c_str(), m_str.c_str() + _Root_end());
    }

    _NODISCARD path relative_path() const {
        size_t _Idx = _Root_end();
        while (_Idx < m_str.size() && _FS_ISSEP(m_str[_Idx])) {
            ++_Idx; // skip extra '/' after root
        }

        return path(m_str.substr(_Idx));
    }
    ...
    _NODISCARD iterator end() const {
        return iterator(*this, m_str.size());
    }

    size_t _Prefix_end() const {
        if (2 < m_str.size() && _FS_ISSEP(m_str[0]) && _FS_ISSEP(m_str[1])
            && !_FS_ISSEP(m_str[2])) { // network name, pick off \\name
            size_t _Idx = 3;
            while (_Idx < m_str.size() && !_FS_ISSEP(m_str[_Idx])) {
                ++_Idx;
            }
            ...
            size_t _Idx = m_str.find(_FS_COLON, 0);
            if (_Idx == m_str.npos) {
                _Idx = 0;
            }
            else {
