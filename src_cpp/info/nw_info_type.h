...
namespace NW
{
	...
	public:
		...
		using tab_t = t_darray<type_info>;
	...
	public:
		...
		size_t align;
	public:
		type_info(type_tc type = NW_NULL, cstr_t tname = NW_NULL, size_tc tsize = NW_NULL, size_tc talign = NW_NULL);
...
namespace NW
{
	...
	public:
		...
		inline cstr_t get_vtype_name() const    { return &get_vtype_info().name[0]; }
		inline size_t get_vtype_size() const    { return get_vtype_info().size; }
		// --setters
		v1nil set_vtype(vtype_tc type);
		template<typename tname> v1nil set_vtype() { set_vtype(type_info::get_type<tname>()); }
		// --predicates
		...
		// --operators
		inline v1nil operator=(owner_tc& copy) { set_vtype(copy.m_vtype); }
		inline v1nil operator=(owner_t&& copy) { set_vtype(copy.m_vtype); }
		template<typename tname>
