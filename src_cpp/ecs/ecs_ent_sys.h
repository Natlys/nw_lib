...
namespace NW
{
	...
	public:
		...
		using ent_tab = dictionary<v1ui, ent_ref>;	// table of t_id and entity references
		using ent_reg = dictionary<v1ui, ent_tab>;	// table of cmp_id and entity containers
		template<class et>
	...
	public:
		...
		inline ent_tab& get_ent_tab(v1ui t_id)              { return m_ent_reg[t_id]; }
		template<class et> ent_tab& get_ent_tab()           { return get_ent_tab(et::get_type_static()); }
		inline ent_ref& get_ent_ref(v1ui t_id, v1ui e_id)   { return m_ent_reg[t_id][e_id]; }
		template<class et> ent_ref& get_ent_ref(v1ui e_id)  { return get_ent_ref(et::get_type_static(), e_id); }
		template<class et> ent<et> get_ent(v1ui e_id)       { return ent<et>(get_ent_ref<et>(e_id)); }
		// --predicates
		inline bit has_ent(v1ui t_id, v1ui e_id)            { return m_ent_reg[t_id].find(e_id) != m_ent_reg[t_id].end(); }
		template<class et> bit has_ent(v1ui e_id)           { return has_ent(et::get_type_static(), e_id); }
	protected:
		ent_reg m_ent_reg;
	};
	...
	public:
		...
		void del_ent(v1ui t_id, v1ui e_id) {
			if (!has_ent(t_id, e_id)) { return; }
			m_ent_reg[t_id].erase(e_id);
		}
		template<class et>
		void del_ent(v1ui e_id) {
			NW_CHECK_TYPE_BASE(a_ent, et);
