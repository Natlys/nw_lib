#ifndef NWL_MEMORY_REFERENCE_H
#define NWL_MEMORY_REFERENCE_H
...
namespace NW
{
	...
	public:
		...
		// --setters
		mem_ref<mtype>& set_ref(mtype& rRef);
		mem_ref<mtype>& set_ref(const mem_ref<mtype>& rmem_ref);
		template<typename vtype>
		mem_ref<mtype>& set_ref(const mem_ref<vtype>& rmem_ref);
		mem_ref<mtype>& reset();
		// --predicates
		...
		template<typename vtype>
		operator mem_ref<vtype>() { mem_ref<vtype> mem_ref(*this); return mem_ref; }
		// --core_methods
		template <typename vtype, typename ... args>
		mem_ref<mtype>& make_ref(args&& ... arguments);
	private:
		mutable mtype* m_ref;
	...
	// --setters
	template <typename mtype>
	mem_ref<mtype>& mem_ref<mtype>::set_ref(mtype& rRef) {
		reset();
		m_ref = &rRef;
	...
	template <typename mtype>
	mem_ref<mtype>& mem_ref<mtype>::set_ref(const mem_ref<mtype>& rmem_ref) {
		reset();
	...
	template <typename mtype> template <typename vtype>
	mem_ref<mtype>& mem_ref<mtype>::set_ref(const mem_ref<vtype>& rmem_ref) {
		reset();
	...
	template <typename mtype>
	mem_ref<mtype>& mem_ref<mtype>::reset() {
	...
	mem_ref<mtype>& mem_ref<mtype>::make_ref(args&& ... arguments) {
		reset();
